...
Subjects Engine::loadGame() {
    ...
    try {
        ...
        if (input_heroInfo.is_open()) {
            ...
            while (input_heroInfo >> name >> gender >> race >> classe >> type >> level >> health >> mana >> money >> experience) {
                hero = subjects.subjectCreation(name, gender, race, classe, type, level, health, mana, money, experience);
            }
...
void Engine::createPersonage() {
    ...
    CreateObjects sword;
    Object drop1 = sword.objectCreation("Rusty", "dagger", "common", 1, 0, 1);
    hero.bag.push_back(drop1);
    saveGame(hero);
}
void Engine::intro() {
    string answer;
